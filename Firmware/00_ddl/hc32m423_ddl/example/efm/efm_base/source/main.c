...
int32_t main(void)
{
    ...
    EFM_SetWinProtectAddr(EFM_SECTOR30_ADRR, EFM_SECTOR31_ADRR);

  
    /* Erase sector 30. */
    EFM_SectorErase(EFM_SECTOR30_ADRR);
    u32Addr = EFM_SECTOR30_ADRR;
    /*******single program mode*******/
    /* program half word */
    ...
    /* program half word */
    if(Ok != EFM_ProgramHalfWordRB(u32Addr, u16Data))
    {
        LED_R_ON();
        LED_G_OFF();
    }
    ...
    u32Addr += 2UL;
    if(Ok != EFM_ProgramWordRB(u32Addr, u32Data))
    {
        LED_R_ON();
        LED_G_OFF();
    }
